...
string RegionValue(string reg)
{
    if (reg == "1") return "x > 0 & y > 0";
    if (reg == "2") return "x < 0 & y > 0";
    if (reg == "3") return "x < 0 & y < 0";
    if (reg == "4") return "x > 0 & y < 0";
    return "Invalid region";
}

Console.WriteLine("Type in your region from 1 to 4: ");
string coord = Console.ReadLine();

string result = RegionValue(coord);
Console.WriteLine($"This region correspondes to these values : {result}");
